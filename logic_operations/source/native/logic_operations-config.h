...
EMP_BUILD_CONFIG( LogicOperationsConfig,
  ...
  VALUE(ENVIRONMENT_FALSE_SIGNALS, size_t, 0, "How many distraction signals should the environment have?"),
  VALUE(ENVIRONMENT_FALSE_SIGNAL_PROB, double, 0.125, "Probability of a random distraction signal each update during evaluation"),
  GROUP(HARDWARE_GROUP, "Hardware Settings"),
  VALUE(ENV_SIGNALS, bool, true, "True: hw can respond to env signals\nFalse: hw cannot respond to env signals"),
  VALUE(ACTIVE_SENSING, bool, true, "True: hw can poll env state w/sensors\nFalse: hw cannot poll env state w/sensors"),
